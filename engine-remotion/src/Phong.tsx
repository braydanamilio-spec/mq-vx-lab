...
const _MUON = ["700", "800", "900"];   // toàn hệ chỉ dùng chữ tiêu đề đậm

// 26/8 — BẢN TRƯỚC CỦA CHÍNH KHỐI NÀY KHÔNG CHẠY. Nó xin cứng `["700","800","900"]` cho mọi phông,
// nhưng Oswald chỉ có tới 700, Anton chỉ có mỗi 400. Xin độ đậm không tồn tại thì `loadFont` NÉM
// LỖI, rơi vào `catch` rồi gọi `f()` trần — tức nạp lại **mọi độ đậm × mọi bộ ký tự**, đúng cái
// mình định tránh, mà log vẫn im vì đã nuốt ngoại lệ. Đo thật sau khi "tối ưu":
//   "Made 29 network requests to load fonts for Oswald" · "Made 40 ... for Manrope"
// Sửa: hỏi `getInfo()` xem phông CÓ những độ đậm nào rồi giao với danh sách muốn; không giao được
// thì lấy độ đậm nặng nhất nó có. Không đoán, không xin bừa.
const _do_dam = (f: Nap): string[] => {
  try {
    const co = Object.keys(((f as any).getInfo?.() || {}).fonts?.normal || {});
    if (!co.length) return _MUON;
    const giao = _MUON.filter((w) => co.includes(w));
    return giao.length ? giao : [co.sort((a, b) => Number(b) - Number(a))[0]];
  } catch {
    return _MUON;
  }
};

const _n = (f: Nap): string => {
  try {
    return (f as any)("normal", {
      weights: _do_dam(f), subsets: ["latin"], ignoreTooManyRequestsWarning: true,
    }).fontFamily;
  } catch {
